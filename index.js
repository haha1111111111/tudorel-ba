...
bot.on("message", function(message) { // when a message is sent
	...
	.setTitle("**INFORMATII**")
        .setColor(1337)
	.addField("Buna!Numele meu este TheHive si sunt aici pentru a va ajuta pe aceasta comunitate! Puteti utiliza comanda /help  pentru a vedea toate comenzile mele! Daca aveti probleme cu mine, puteti contacta un administrator! :smile:", "Acestea sunt informatiile mele!")
...
if (command == "suggest") {
   ...
   let logs = message.guild.channels.find("name", "sugestions").send(`\`\`\`${suggest}\`\`\`\nSugestia a fost scrisa de ${message.author.tag}`).then(msg => {
   msg.react("✅")
   msg.react("❎")
   }) 
